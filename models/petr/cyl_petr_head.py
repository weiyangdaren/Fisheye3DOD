...
class CylindricalPETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is CylindricalPETRHead):
        ...
        super(CylindricalPETRHead, self).__init__(
    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        ...
        h_max, h_min = self.horizontal_range
        v_max, v_min = self.vertical_range
        y_max, y_min = math.tan(v_max), math.tan(v_min)

        # Create angular grid φ and y grid
        phi = torch.linspace(h_min, h_max, W, device=img_feats[0].device)  # (W,)
        y = torch.linspace(y_min, y_max, H, device=img_feats[0].device)   # (H,)
        phi_grid, y_grid = torch.meshgrid(phi, y, indexing='xy')  # shape: (H, W)

        phi_grid = phi_grid[None, :, :].expand(D, H, W)
        y_grid = y_grid[None, :, :].expand(D, H, W)
        coords_d = coords_d.expand(D, H, W)

        xs = coords_d * torch.sin(phi_grid)
        ys = coords_d * y_grid
        zs = coords_d * torch.cos(phi_grid)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is CylindricalPETRHead:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        masks = torch.zeros((batch_size, num_cams, x.size(3), x.size(4)),
                           dtype=torch.bool,
                           device=x.device)
